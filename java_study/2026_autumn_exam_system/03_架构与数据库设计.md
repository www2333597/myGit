...
状态：设计待实现。这里的 SQL 用于讲解思路，没有执行建表、迁移或修改数据库。
...
课程提到的“Spring 三件套”是 Spring、Spring MVC、Spring Boot。常说的 SSM 则是 Spring、Spring MVC、MyBatis，两种组合里的第三个名字不同。

## 后续代码目录

以下目录仅表示后续生成位置，当前不表示已经有对应 Java 文件。
...
字段类型是设计建议，生成 DDL 时再逐项核对长度、默认值、外键和索引。所有业务表使用 InnoDB；时间由服务端生成，统一使用北京时间展示。
...
为什么不把学生答案另放第七张表：本方案一次答卷的一个题目只有一个最终答案，可放在对应快照行；后续若需要逐次保存答题轨迹，再增加独立记录。

为什么题目使用删除标志：教师从当前题库删除题目时，历史来源关联仍存在；历史判分和展示读取自己的快照，不依赖当前题库。
...
索引设计围绕实际 SQL 验证；小数据返回快不能证明索引有效。分页统一 page 从 1 开始、pageSize 默认 10、最大 50，计算 offset 后使用绑定参数。排序字段只允许后端定义的选项。
...
锁同一个选课行的原因：两个“开始考试”请求即使来自不同线程，也必须依次完成检查和创建。仅做“先查询有没有，再插入”存在同时查到没有的情况。唯一约束作为额外的数据库约束，不能替代完整事务规则。
...
用 Map 按答卷明细 ID 查找答案，便于验证重复项和归属；所有 ID 都来自服务端生成的答卷。这里使用你已经学过的集合、条件和循环即可。纯判分函数本身不访问数据库，方便单独测试。

## 登录和浏览器交互

计划用 HttpSession 保存最少的用户 ID/角色信息，以 Cookie 携带会话标识。登录成功更新会话标识，退出使会话失效；密码使用 BCrypt 哈希校验，可使用 Boot 版本管理下的 spring-security-crypto，不自行编写密码加密算法。

前端开发时优先统一通过本地代理请求 `/api`。若采用浏览器直连不同端口，需要明确允许的源、携带 Cookie 的配置，并验证 OPTIONS 预检；Apifox 成功不代表浏览器 Cookie 或跨域设置必然正确。

Cookie 登录方案需要配套服务端 CSRF Token 校验：读取 token 的接口供前端初始化使用，登录、退出及其他写入请求携带 `X-CSRF-Token`。不能把关闭浏览器检查当作联调方案。具体实现与案例随登录批次一起讲解。

## 数据初始化与开发验证

使用独立的学习数据库名称，例如 `exam_study`，数据库密码通过本地环境变量或未公开的本地配置提供。初始化脚本与演示数据手动执行，应用启动时不自动删表重建。
